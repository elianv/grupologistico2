...
class Bodegas extends CI_Controller{
    ...
    function modificar_bodega(){
        ...
        if($this->session->userdata('logged_in')){
            ...
            $this->load->library('form_validation');
            $this->form_validation->set_rules('nombre', 'Nombre','trim|required|xss_clean|callback_check_unico_editar');
    ...
    function check_unico_editar($bodega)
    {
        $result = $this->Bodegas_model->existe_nombre_editar($bodega , $this->input->post('codigo_bodega'));

        if($result){
            $this->form_validation->set_message('check_unico_editar','El Nombre que ingresa se encuentra en el sistema, intente con otro.');
            return false;
        }
        else{
            return true;
